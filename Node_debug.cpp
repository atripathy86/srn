...
inline void RouterNode::trim_routing_table_length(){
	...
	printf("Ticks %u: Node %u: RouterNode::trim_routing_table_length(), size before trimming is %d, routing_table_length_floor is %d\n", 
		If_ptr->scheduler_ptr->now(), id, routing_table.size(),routing_table_length_floor );
print_routing_table();print_routing_table_index();
	...
	{
		printf(" index %u : tag %u, ", fi, *routing_table_index_itr );
	...
	printf("Ticks %u: Node %u: RouterNode::trim_routing_table_length(), size after trimming is %d\n", If_ptr->scheduler_ptr->now(), id, routing_table.size() );
...
inline void RouterNode::trim_routing_table_specific_width(RoutingTableRow * row_to_trim_ptr){
				...
				routing_table_entry_itr++  ) 
	{
					...
					row_to_trim_ptr->num_resource_nodes > routing_table_min_num_resource_nodes   ) 
		{
			...
			row_to_trim_ptr->num_resource_nodes--;
			fi--;
	...
	//destinations_to_trim.erase( destinations_to_trim.begin(), routing_table_entry_itr );
...
inline	int ResourceNode::verify_against_description(Query &submitted_query) {
	...
	for(int fi= submitted_query.description.size();fi>0;fi--)
	{
		...
		//if ( description.find( (*description_itr).first ) != description.end() )
		if ( description.find( (*description_itr) ) != description.end() )
		{

#ifdef DEBUG_NODE
		printf("Ticks:%u : Node %u: ResourceNode::verify_against_description():Received by node %u, type %u, match found for the query\n", 
				If_ptr->scheduler_ptr->now(),id ,id, node_type);
#endif
			return 1;
		}
...
#ifdef DEBUG_NODE
		printf("ResourceNode::verify_against_description(): Reached here 1: Received by node %u, type %u, no match found for the query\n", id, node_type);
#endif
...
//printf("Reached here 2a\n"); 			
 				...
 				//Tag already exists, so just enter in the suitable row
 				//((*(routing_table).find(*description_itr)).second->destinations).insert(passed_node_ptr);
				(routing_table_row_obj_ptr->destinations).insert(passed_node_ptr);
				//Increment the resource counter
				if (passed_node_ptr->node_type == RESOURCE_NODE) routing_table_row_obj_ptr->num_resource_nodes++;
//printf("Reached here 3a\n"); 			
...
inline void RouterNode::print_routing_table()
	...
	//
					...
					{
						cout<<"Node id : Row i : Description tag : List of Node id - node_type that are attached....."<<std::endl;
						int i=0;
						while (print_routing_table_row_itr != routing_table.end())
						{
							cout<<"Node :"<<id<<": Row "<<i<<" : Tag "<<(*print_routing_table_row_itr).first;
							...
							{
								cout<<" id "<<(*print_routing_table_entry_itr)->id<<" - type "<<(*print_routing_table_entry_itr)->node_type<<" ,";
